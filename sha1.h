...
#if defined(WIN32) || defined(Q_OS_WINDOWS)
...
public:
	...
	void UpdateHash(const uint8_t *data, uint32_t len);

	bool HashFile(const std::string & filename);

	// Finalize hash -- call this after done reading data
	void Final();

	// Report functions: as pre-formatted and raw data
        std::string ReportHash(ReportType t = REPORT_HEX_SHORT) const;
	void GetHash(uint8_t *puDest) const;
