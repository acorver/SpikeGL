...
#define SHA1_MAX_FILE_BUFFER 131072
...
void SHA1::Transform(uint32_t *state, const uint8_t *buffer)
{
...
void SHA1::UpdateHash(const uint8_t *data, uint32_t len)
{
	uint32_t i, j;
...
bool SHA1::HashFile(const std::string & fileName)
{
	...
	for(i = 0; i < ulBlocks; i++)
	{
		...
		UpdateHash((uint8_t *)uData, nread);
	}
...
std::string SHA1::ReportHash(ReportType uReportType) const
{
	unsigned char i;
	char szTemp[16];
        char szReport[512];
...
void SHA1::GetHash(uint8_t *puDest) const
{
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Please pass a filename to generate the sha1 hash of\n");
        return 1;
    }
    SHA1 sha1;
    if (!sha1.HashFile(argv[1])) {
        fprintf(stderr, "Error generating hash of %s\n", argv[1]);
        return 2;
    }

    printf("%s  %s\n", sha1.ReportHash().c_str(),argv[1]);
    return 0;    
}
